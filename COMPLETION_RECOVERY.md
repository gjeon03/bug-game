...
**어느 쪽인지 재지 않았다.** 재지 않은 것을 적어둔다 — 이번 세션에 같은 자리에서
세 번 틀렸고, 네 번째로 추측을 적지 않는다.

### 다음 사람에게

먼저 `run.status`와 `document.visibilityState`를 긴 대기 중에 함께 찍어라. 한 줄이면
어느 쪽인지 갈린다. 그다음에야 밀집 프레임을 시도할 것.
