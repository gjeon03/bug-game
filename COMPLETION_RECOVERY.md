...
## §62 — 미끼를 되돌린다. 그 75초는 죽은 시간이 아니었다

§61은 박멸 쿨다운 75초를 "죽은 대기"로 읽고 플레이어가 건너뛸 수 있게 만들었다. 그 전제가 틀렸다.

먼저 구조부터 고쳤다. 미끼 로직이 `boot.ts`의 명령 스위치 안에 있어서 **밸런스 봇이 닿을 수 없었다** —
...
디자이너가 옳게 지적한 것은 "쓸모없는 시간"이 아니라 **"할 게 없는 시간"**이었고, 둘은 다른 처방을
요구한다. 건너뛰기는 처방이 아니다 — 누르면 무조건 지는 버튼이고, **함정을 출하하는 것은 대기를
출하하는 것보다 나쁘다.** 되돌렸다(`git revert 46ec383`). 긴급 소환(§60)은 그대로 남는다.

### 중간에 잡힌 진짜 결함

첫 실패 때 원인을 오진했다. 봇이 매 틱 미끼를 부르는데 `sweepCooldown`은 박멸 직후 다시 차므로
조건이 즉시 참이 되어 **무한 재미끼**가 됐다 — 플레이어가 B를 연타해도 같다. 스윕당 1회 가드를 넣어
막았지만, 그걸 고쳐도 여전히 졌다. 가드는 옳았고 설계가 틀렸다.

### 다음에 이 대기를 다룰 방법

건너뛰지 말고 **채울 것**. 그 75초에 군체가 하는 일(수리·비축)을 플레이어가 개입할 수 있는 형태로
드러내면, 대기가 사라지지 않고도 결정이 생긴다. 수리 우선순위, 비축 배분, 대피 배치 같은 것들이
후보다. 이것은 다음 패스의 과제로 남긴다.

### 절차 메모

- `pnpm test`는 런 길이·승리 테스트를 **포함하지 않는다**. `test:slow`만 `tests/unit/run.test.ts`를
  돌린다. 이 세션의 앞선 패스들에서 "89/89 통과"를 근거로 삼은 것 중 봇 경로에 관한 것은 그만큼
  약한 증거였다. 시뮬레이션을 건드렸으면 `pnpm test:slow`를 돌릴 것.
- 되돌림이 `artifacts/evidence/bait-verified/`도 함께 지웠다. 되돌린 기능의 증거이므로 남기지
  않았고, 그 측정 내용은 위 표에 보존했다.
