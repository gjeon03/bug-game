...
### 다만 이 되돌림은 §28과 같은 함정일 수 있다

6런 표본은 10.82~49.04분이고 중앙값이 낮아졌다고 말하기 어렵다. 깨진 것은
**한 시드**이고, `run.test.ts`의 런 길이 게이트는 정확히 그 한 시드를 본다 —
`peakPopulation`에서 고친 것과 같은 단일 시드 문제이며 런 길이 쪽은 아직 안 고쳤다.

그러니 순서는 이렇다: **(1) 런 길이 단언을 분포 위에서 재유도하고, (2) 그다음 이 세
줄을 다시 넣는다.** §30이 봉인 지역 루틴에서 배운 것과 같은 순서이고, 그때 이 순서를
지키지 않아 옳은 수정을 한 번 잃었다.

붙일 위치는 `kitchen.ts`의 `bin.inside.food` · `fridge.seal` · `table.ring` 세 자원의
`kind:` 바로 앞, `hidden: true,` 한 줄씩이다.
