...
| --- | --- | --- | --- |
| W2 오버레이 | 확인 | 확인 | 없음 |
| W3 일꾼 변주 | 확인 | 확인 (`[0.88, 1.0, 1.101]`) | 부분 — 3체 |
...
| 따라서 페이드가 §3의 150~300 ms 안에 있다 | **추론** — 타임베이스 비율과 상수에서 따라 나오지만 프레임에서 재지 않았다 |

페이드 시간을 직접 재려고 144 Hz에서 오클루더 불투명도를 40초 추적했으나 아무것도
페이드하지 않았다. 정찰병이 싱크대 밑에서 시작하므로 카메라와 몸 사이에 프롭이
오는 구도가 그 시간 안에 나오지 않는다. 밀집 프레임(§45)에서 배운 것과 같다 —
구도를 만들려면 게임을 그쪽으로 몰아야 한다.

다음 사람에게: `walkTo`로 정찰병을 캐비닛 뒤쪽으로 보낸 뒤 같은 탐침을 붙여라.
§45의 스크립트가 그 이동을 이미 한다.
