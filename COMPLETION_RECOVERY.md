...
## §57 — 정찰병을 키웠다. 첫 시도는 무효였고 재관측이 그것을 잡았다

사용자가 "바퀴벌레가 너무 작다"고 지적했고 §55에서 24 px(1200 px 폭의 2 %)로 확정했다.

**첫 시도는 no-op이었다.** `REFERENCE_BODY_MM`을 35 → 50으로 올렸는데, 그 상수는 크기가 아니라
**나눗수**다: `scale = bodyMm / REFERENCE_BODY_MM`이고 정찰병은 `bodyMm`을 넘기지 않아 기본값이
기준과 같아진다 — 어떤 수를 넣어도 scale은 정확히 1이다. 일꾼도 같은 나눗수를 쓰므로 26 → 37로 함께
옮긴 것이 비율 0.74를 그대로 유지해 역시 아무것도 바뀌지 않았다. 캡처의 0.4186 → 0.4234 차이는 내
변경이 아니라 실행 간 잡음이었고, 하마터면 그것을 "43 % 증가"의 증거로 보고할 뻔했다.
**같은 영역을 4배로 잘라 눈으로 비교한 재관측이 막았다.**
그 무효 실행의 증거는 `artifacts/evidence/scout-scale-50mm/`에 그대로 남겨 둔다 — §8에 따라 지우지
않으며, "이름은 50 mm인데 화면은 35 mm와 같은" 기록 자체가 이 항목의 근거다.

올바른 지렛대는 `bodyMm`을 명시적으로 넘기는 것이다. `roaches.ts`에 `SCOUT_BODY_MM = 50`을 두고
정찰병에만 전달했다. 기준(35)과 일꾼(26–30)은 원래대로 되돌렸다.

카메라는 쓰지 않았다. `CAM_DEFAULT_MM`은 1320 → 1900으로 **올린** 값이고 그 이유가 "비평가가 방을
알아볼 수 없었다"이므로, 당기면 이미 닫은 결함을 다시 연다. 몸 크기는 프레임 구성을 건드리지 않는다.

검증 (`artifacts/evidence/scout-50mm-verified/`):

```
00-scout-35mm-vs-50mm.png   같은 뷰포트·같은 시작 위치, 4배 확대 나란히 비교
frame luminance: 0.4199     (밴드 내)
console errors 0 · warns 0 · failed 0 · external 0 · missing props 0
draw calls 323 · triangles 156262   ← 변경 전과 동일. 균일 스케일이라 성능 비용 0
20 restarts identical: true · GPU 객체 누수 0
typecheck 통과 · 단위 89/89 · 빌드 성공
```

일꾼은 26–30 mm 그대로라 정찰병이 화면에서 가장 큰 바퀴벌레라는 위계가 생겼다.

**이 세션의 교훈이 하나 더 늘었다.** 계측기를 스물다섯 번 틀렸고, 이번엔 계측기가 아니라 **변경 자체가
무효**였는데 통과하는 지표(빌드 해시 변화, 잡음 수준의 휘도 차이)가 성공처럼 보였다. 픽셀을 직접 보지
않았다면 기록에 거짓이 남았을 것이다.

## §58 — 현재 HEAD의 아트 점수는 58. 찬장 손잡이를 달았고, 문은 아직 평평하다

현재 HEAD에서 아트 감독 페르소나를 새로 돌렸다(이전 47은 4인 패널·구버전 커밋 기준이라 비교 불가).

**58 / 100.** P0 하나, P1 둘:

1. **P0 — 찬장 문이 큰 평면 갈색 판때기.** 판 깊이도, 하드웨어도, 결도 없다. §7이 금지한 "가전을
   평면 벽으로 그리기"와 같은 실패다.
2. P1 — 바닥이 타일 줄눈도 마모도 없는 반복 사선 텍스처. 화면에서 가장 큰 면이 주방으로 읽히지 않는다.
3. P1 — 좌상단 광원이 기구도 창도 없는 흰 덩어리. 배치된 조명이 아니라 블룸 아티팩트로 읽힌다.

긍정: 정찰병 메시 자체는 "진짜 잘 만들어졌다"고 평가받았다(§57의 확대가 그것을 보여준다).
**환경이 히어로를 따라오지 못하는 것이 아트 점수의 핵심이다.**

### 이번에 고친 것: 바 손잡이

돌출 판은 이미 있었다. 눈이 찾지 못한 것은 **하드웨어**다. 손잡이 없는 문은 사각형이고, 베벨로는
고쳐지지 않는다 — 베벨은 문과 같은 색이지만 강철 손잡이는 아예 다른 재질이기 때문이다. `steelBrushed`는
무광 문이 절대 못 잡는 각도로 스포트라이트를 받으므로, 카메라를 따라 움직이는 하이라이트가 문마다
생긴다. 그게 찬장 줄을 칠해 놓은 벽 하나로 보이지 않게 만드는 것이다.

검증 (`artifacts/evidence/cabinet-pulls/`):

```
00-doors-before-after.png   같은 샷 크롭 비교 — 바와 지주가 실제로 렌더된다
frame luminance: 0.4168     (밴드 내)
console errors 0 · warns 0 · failed 0 · missing props 0
draw calls 323 -> 328 · triangles 156262 -> 158758 (+1.6 %)
geometries 121 -> 124 · meshes 357 -> 393 · 20회 재시작 동일, 누수 0
typecheck 통과 · 단위 89/89 · 빌드 성공
```

**정직하게: 이것으로 P0가 닫히지 않는다.** 비교 이미지에서 문은 여전히 평평하게 읽힌다. 하드웨어는
...
4. 나머지 두 페르소나(게임플레이·기술)를 현재 HEAD에서 돌려 3인 점수를 완성한다.

## §59 — Codex 3인 진단. 게임성 43이 최저이고, 이것이 아트 계획을 폐기시킨다
...
- 디자이너 P0 — 10초마다 의미 있는 결정이 없다. 길을 그린 뒤 일꾼은 스스로 배정되고 물류는
  방치돼도 돌아간다 (`src/colony/workers.ts`, `src/colony/routes.ts`).
- 디자이너 P0 — 25분은 저작된 상승이 아니라 소모 튜닝이다. 박멸 쿨다운 75초 대기로 끝날 수 있다
  (`src/colony/step.ts`, `src/colony/progression.ts`).
- 디렉터 #1 — 수동적 자원 축적이 아니라 상승하는 결정을 만들어야 한다.

### 이 진단이 §58의 계획을 폐기한다

디렉터의 문장이 내 다음 세 항목을 정면으로 반박한다:

> **"찬장 결과 바닥 마모는 공간이 선택을 만들어낸 뒤에야 의미가 있다."**

§58은 문 판 깊이 → 바닥 줄눈 → 광원 기구 순서를 다음 작업으로 적었다. **그 순서를 내린다.**
지난 두 패스를 정찰병 크기와 찬장 손잡이에 썼는데, 가장 낮은 점수는 43이고 아트가 아니었다.
가독성만은 세 페르소나가 모두 지적했으므로 남긴다.

### 새 우선순위 (§58의 목록을 대체한다)

1. **결정 루프** — 일꾼 우선순위, 긴급 소환, 미끼, 희생 라인. 현재 위협 대응 어휘가 사실상
   "지우고 다시 긋기" 하나뿐이다 (`src/colony/household.ts`, `src/game/input.ts`).
2. **저작된 상승** — 75초 죽은 대기를 없애고 25분을 튜닝이 아니라 각본으로 만든다.
3. **가독성** — 경로·일꾼·화물·위험·통행 가능 면이 창백한 오버레이로 뭉개진다 (3인 합의).
4. **버그 P1 둘** — `routes.ts:103` A* 복구가 실패해도 장애물 뒤 점을 그대로 붙여 통과 불가
   구간을 가진 "정상처럼 보이는" 경로를 만든다. `progression.ts:393` 번식이 공급되지 않는 거점까지
   세어, 어떤 길에도 닿지 못하는 일꾼을 낳는다.

### 버릴 것

디렉터: 나머지 네 방을 영구히 포기하고 그 진행 가정을 보존하지 말 것. 미래 가치가 아니라
매몰비용이며, 목표·밸런스·아키텍처·로드맵에서 그 개념적 중력을 제거해야 한다.

### 운영 메모

- Codex 실행 로그에 `--dangerously-bypass-hook-trust is enabled` 경고가 찍혔다. 내가 준 플래그가
  아니라 플러그인 기본값이고, 훅이 검토 없이 실행됐다는 뜻이다.
- 첫 시도는 `stdin`이 파이프에 물려 codex가 표준입력을 기다리다 10분 타임아웃으로 죽었다.
  **계측기 실패 26번째** — `< /dev/null`로 해결. 이번에도 게임이 아니라 도구였다.

## §60 — 긴급 소환. `panic()`은 이미 있었고 플레이어만 부를 수 없었다

§59가 게임성 43의 P0로 "위협 대응 어휘가 사실상 지우고 다시 긋기 하나뿐"을 지목했다.
`workers.ts`를 열어 보니 **`panic()`이 이미 존재했다** — 일꾼이 짐을 버리고 거점으로 뛰게 만드는
바로 그 함수인데, 호출자가 가정(household)뿐이었다. 플레이어는 그 일이 벌어지는 것을 볼 수만 있고
일으킬 수는 없었다. 이 세션의 최대 수확들과 정확히 같은 패턴이다: **있는데 실행되지 않는 기능.**

키에 묶으니 위협 예고가 값이 붙은 질문이 된다. 소환하면 분사 반경에 선 일꾼은 살지만 운반 중인
부스러기는 전부 버려지고, 쿨다운(45초) 때문에 헛짚으면 다음 진짜 위협에서 못 뛴다. 결정이고,
런 내내 반복된다.

키보드 전용 계약(§1a)에 따라 `Q`에 묶고 한글 자판 `ㅂ`도 매핑했다. §4의 실패("카탈로그가 완전해도
쓰이지 않으면 소용없다")를 피하려고 `help.recall`을 `hud.ts`의 도움말 배열에 실제로 넣었다.

### 실브라우저 검증 (Playwright MCP, `artifacts/evidence/recall-verified/`)

```
Q 1회      log.recall.ordered {count: 2}   recallReadyAt 36 -> 81 (= +45, 정확)
Q 2회 즉시  log.recall.cooling               거부 경로 동작
도움말      "긴급 소환" 표시됨               떠도는 영어 없음
frame luminance 0.4186   콘솔 오류 0 · 경고 0 · 실패 요청 0 · 누락 프롭 0
draw calls 328 · triangles 158758   ← §58과 동일. 렌더 비용 0
20회 재시작 동일 · GPU 누수 0 · typecheck · 린트 · 단위 89/89 · 빌드
```

부수 수정: 휘도 게이트의 `new Image()`가 Node 린트 설정에서 미정의 전역이라 실패했다.
`document.createElement('img')`로 바꿨다. §56에서 넣은 게이트의 잔여 결함이다.

### 남은 것

이것은 P0의 **한 갈래**다. 디자이너가 함께 지목한 미끼·희생 라인·일꾼 우선순위는 아직 없고,
"저작된 상승"(75초 죽은 대기)도 그대로다. 게임성 재채점은 그것들을 넣은 뒤에 의미가 있다.

## §61 — 미끼. 대기를 이름 붙이는 것과 없애는 것은 다르다

§59가 게임성 43의 두 번째 P0로 "25분은 저작된 상승이 아니라 소모 튜닝이고, 박멸 쿨다운 75초
대기로 끝날 수 있다"를 지목했다. `step.ts:240-252`를 열어 보니 **이미 측정돼 있었다**: 모든 조건은
169초에 충족되고 런은 242~252초에 끝나므로 그 사이 75~77초, 런의 31~37 %가 쿨다운이 줄어드는 것을
보는 시간이다. 그때의 대응은 `blocker.extermination`이라는 이름을 붙여 목표 패널이 침묵하지 않게 한
것이었다. **침묵은 사라졌지만 대기는 그대로였고, 디자이너는 그것을 여전히 P0로 채점했다.**

미끼는 대기를 정직한 방식으로 없앤다 — 플레이어가 자기 조건으로 끝낼 수 있게. 일부러 흔적을 남기면
사람이 나중이 아니라 지금 온다. 다만 더 세게 온다: 방금 심은 증거가 `severity`를 정하는 바로 그
`pressure`에 들어가기 때문이다. 조용히 기다릴 것인가, 군체가 아직 강할 때 끝낼 것인가.

최종 장이고 쿨다운이 실제로 남아 있을 때만 쓸 수 있게 걸었다. 측정이 서술하는 상황이 정확히 그것이고,
더 이르게 열면 아무도 측정하지 않은 밸런스 비용을 가진 다른 기능이 된다.

### 검증 (`artifacts/evidence/bait-verified/`)

```
00-controls-ko.png   조작 커튼에 "Q — 긴급 소환", "B — 미끼" 둘 다 렌더. 한국어, 토후 없음
B 조기 입력          log.bait.pointless — 거부 경로 동작 (실브라우저)
frame luminance 0.4193   콘솔 오류 0 · 경고 0 · 실패 요청 0 · 누락 프롭 0
draw calls 328 · triangles 158758   ← §60과 동일
20회 재시작 동일 · GPU 누수 0 · typecheck · 린트 · 단위 89/89 · 빌드
```

**검증되지 않은 것 (정직하게)**: 미끼의 **수락 경로**는 최종 장에서만 발동하는데, 실제 런으로 거기까지
가서 확인하지 못했다. 타입체크와 단위 테스트는 통과하지만 그것은 §8이 말하는 증거가 아니다.
숨은 상태를 조작해 최종 장으로 점프하는 것은 "실제 플레이어가 쓸 수 있는 조작만"이라는 규칙을
어기므로 하지 않았다. **다음 패스에서 봇으로 완주해 수락 경로와 밸런스 영향을 확인해야 한다.**

### 계측기 실패 27번째

`document.getElementById('hud').innerText`로 도움말을 검사했더니 두 항목 모두 false가 나왔다.
커튼이 `#hud` 밖 요소라 잡히지 않은 것이고, 스크린샷을 찍어 보니 둘 다 멀쩡히 렌더되고 있었다.
**DOM 질의가 화면의 대용물이 아니다** — 이 세션에서 같은 교훈을 캔버스 판독(§55)으로 이미 배웠다.
