...
| B0  | **A complete run is proven for the concealment build.** `shadow`, seed 4242, bathroom skipped, driven only through functions the input layer calls: **WON in 9.7 min** — gates kitchen→hallway 1.0 / living 3.1 / bedroom 8.0, **12 footholds across four regions**, peak population 58, 6 sightings, 12 workers lost, **1 extermination sweep survived**, longest plateau 5.7 s.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **CLOSED for one build**                   |
...
| B6  | **The `brood` build is supply-limited and does not complete.** Seed 20260805: capacity 30 but peak population 14, 277 deliveries, `driedUp: 0`, **zero threats and every region at alert 0** — and it still ends moisture 0, population 0. Three attempted fixes were measured and none moved it: cutting brood's moisture cost 4.2→2.4 (food 17→32, otherwise identical), a bot re-router (**byte-identical output — it never fired**, its guard returns early whenever any moisture route exists), and a starvation grace period (`lost: 76` unchanged, because the colony sits at zero _continuously_ rather than spiking). The mechanism is now known: moisture income is below consumption from the first minute for a specialization whose whole purpose is growth. Fixing it means re-deriving moisture supply per region against brood's growth curve, not another constant. | **OPEN — mechanism identified, not fixed** |
...
### The finale was a metronome

`updateFinal` rolled a per-tick probability with no cooldown. At 60 Hz that is a whole-home
extermination roughly every thirteen seconds — **measured: 125 sweeps in a single run**. The event
the victory screen describes had no weight at all, and the recovery loop it is supposed to create
never existed because there was no gap to recover in.

Now: a 110 s floor between sweeps, the first never before 90 s, and each one hits harder than the
last. Pressure decides how soon within the window, never how often.

---

### What thirteen balance measurements actually established

Run length is not a constant to sweep, and neither is difficulty. Every failure across thirteen
configurations had the same shape: **one resource at zero while the other overflowed**, and the
cause was never the amount, always the distribution. The one change that ever moved run length was
putting food where the colony starts. Three separate "fixes" to the brood build produced byte-
identical output, which is its own useful signal — identical numbers from a changed build means the
changed code never executed.

Two of my own repairs made things measurably worse and were reverted with their numbers kept:
a 2.5x gate-cost rise (won 24.5 min → not won at 45), and making idle exposure costlier than moving
(sightings 65 → 100, because the bot stands still at gates).
