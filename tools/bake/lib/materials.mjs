...
/*
 * NEGATIVE RESULT — procedural surface detail was built, measured, and removed. Do not rebuild it
 * without reading this.
 *
 * Three escalating attempts to give flat surfaces micro-detail:
 *   1. `roughnessMap` only. Invisible — under a soft environment map a matte surface barely changes
 *      appearance with roughness.
 *   2. Promoted to `map` (albedo). Still invisible, for a non-obvious reason: `ExtrudeGeometry`
 *      emits UVs in raw WORLD coordinates, not normalised 0..1, so `repeat: 5` on a 240-unit
 *      counter tiled the pattern 1200 times and compressed it below one pixel. Lathe and sphere
 *      geometry do use 0..1 UVs, so the same number means two different things depending on how the
 *      prop was built.
 *   3. Repeat retuned to tiles-per-world-unit. Now visible — as a regular grid, because the canvas
 *      texture's own tile seams lined up. That is "repeated procedural noise", which is on the
 *      banned list, and it grew the packed sheet from 321 kB to 1155 kB.
 *
 * The props were already reading correctly without any of it: form, material response and contact
 * shadow come from geometry and the light rig, which is where they should come from. Large flat
 * architecture gets its variation from the runtime tiling layer instead, which already randomises
 * across a 640-unit period specifically so no grid is visible.
 */
...
export const ceramicFoot = (color = 0xc9ccc8) => std({ color, metalness: 0.0, roughness: 0.78 });
